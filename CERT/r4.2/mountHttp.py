...
httpFile, httpFilePath = crear_escribirArchivo(outputPath,"joinedWebAccess", "_", "csv")

stringInsiders=cargar_insiders(outputPath+"\\"+insiderFile)

# Generar la cabecera
clave = ","
httpFile.write("insider"+clave+"user"+clave+"httpText\n")
...
for name in OutputFileList:
    if name.__contains__("http_"):
        user = "".join( ( "".join(name.split("http_")) ).split(".txt") )
        HttpFileList.append(name)
        userList.append(user)

for filename in HttpFileList:
    # Previous params
    isInsider = False
    content = ""
    # Get username
    user = "".join( ( "".join(filename.split("http_")) ).split(".txt") )
    print(user)
    # Is Insider?
    if stringInsiders.__contains__(user):
        isInsider = True
    # Extract content
    with open(outputPath+"\\"+filename) as userHttpFile:
        for line in userHttpFile:
    ...
    # Save content
    with open(httpFilePath, 'a', encoding='utf-8') as http_file:
        http_file.write(str(int(isInsider))+","+user+","+content+"\n")